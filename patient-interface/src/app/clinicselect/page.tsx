...
export default function ClinicSelect() {
  ...
  function handleClinicChange(e: any) {
    ...
    setFilteredDentists(
      dentists.filter((dentist) => dentist.clinic === e.target.value)
    );
  }
  ...
  function handleSelect() {
    ...
    router.push(
      `/booking/?clinic=${selectedClinic}&dentist=${selectedDentist}`
    );
  }
        ...
        <form className="bg-white dark:bg-slate-700 shadow-md rounded px-8 pt-6 pb-8 mb-4">
          ...
          <div className="mb-4">
            ...
            <select
              ...
              id="dentists"
              disabled={!dentistActive}
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            >
              <option selected>Choose a dentist</option>
              {filteredDentists.map((dentist) => (
                <option key={dentist.personnummer} value={dentist.personnummer}>
                  {dentist.name}
                </option>
              ))}
            </select>
